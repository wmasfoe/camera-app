...
class CameraService: NSObject, ObservableObject {
    ...
    func setExposure(normalized value: Float) {
        guard let device = currentDevice else { return }
        let range = device.activeFormat.minExposureCompensationIndex...device.activeFormat.maxExposureCompensationIndex
        let index = value * (range.upperBound - range.lowerBound) / 2 + (range.upperBound + range.lowerBound) / 2
    ...
    func updateExposureRange() {
        guard let device = currentDevice else { return }
        let minEV = device.activeFormat.minExposureCompensationIndex
        let maxEV = device.activeFormat.maxExposureCompensationIndex
