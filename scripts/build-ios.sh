...
if [[ "$BUILD_SIMULATOR" == "true" ]]; then
    ...
    mkdir -p "$SHARED_CORE/target/universal-sim/release"
    lipo -create \
        "$SHARED_CORE/target/$SIM_ARM64_TARGET/release/libcamera_shared_core.a" \
        "$SHARED_CORE/target/$SIM_X86_64_TARGET/release/libcamera_shared_core.a" \
        -output "$SHARED_CORE/target/universal-sim/release/libcamera_shared_core.a"
fi
...
cargo run --bin uniffi-bindgen -- generate \
    --library "$SHARED_CORE/target/$DEVICE_TARGET/release/libcamera_shared_core.a" \
    --language swift \
    --out-dir "$OUTPUT_DIR/"
...
echo ""
echo "   Device library:"
echo "     $SHARED_CORE/target/$DEVICE_TARGET/release/libcamera_shared_core.a"

if [[ "$BUILD_SIMULATOR" == "true" ]]; then
    echo ""
    echo "   Simulator (universal) library:"
    echo "     $SHARED_CORE/target/universal-sim/release/libcamera_shared_core.a"
fi
